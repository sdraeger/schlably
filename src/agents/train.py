...
def final_evaluation(config: dict, data_test: List[List[Task]], logger: Logger):
    ...
    logger.log_wandb_artifact(
        {"name": "agent_model", "type": "model"},
        file_path=best_model_path.with_suffix(".pkl"),
    )
    ...
    evaluation_results = test.test_model_and_heuristic(
        config=config,
        model=best_model,
        data_test=data_test,
        logger=logger,
        plot_ganttchart=False,
        log_episode=True,
    )
...
def training(
    config: dict,
    data_train: List[List[Task]],
    data_val: List[List[Task]],
    logger: Logger,
) -> None:
    """
    ...
    inter_test = intermediate_test.IntermediateTest(
        env_config=config,
...
def main(config_file_name: dict = None, external_config: dict = None) -> None:
    ...
    split_random_seed = (
        config["seed"] if not config.get("overwrite_split_seed", False) else 1111
    )
    ...
    test_data, val_data = train_test_split(
        test_data,
        train_size=config.get("test_validation_split"),
...
def get_parser_args():
    ...
    parser = argparse.ArgumentParser(
        description="Train Agent in Production Scheduling Environment"
    )
